...
template = '''
class Instruction{name_title}(Instruction):
    ...
    def __init__(self{other_param}):
        super(Instruction{name_title}, self).__init__()
        self.code = '\\x{code}'{init_sentences}
'''
init_template = 'self.{param_name} = {param_name}'
all_class = []
for m in re.finditer(r'if ch == 0x(\w+):[\n\W\w]*?print\((.*)\)', code):
    icode, out_content = m.group(1, 2)
    parts = out_content.split(',')
    name = parts[0].strip('"')
    if len(parts) > 1:
        res_parts = list(map(lambda x: x.strip(), parts[1:]))
        init_sentences = '\n        ' + '\n        '.join(map(
            lambda s: init_template.format(param_name=s),
            res_parts
        ))
    else:
        res_parts = None
        init_sentences = None
    # print(icode, name, res_parts, init_sentences)
    print(template.format(
        code=icode,
        name=name,
        name_title=name.title(),
        other_param=(', ' + ', '.join(res_parts) if res_parts else ''),
        init_sentences=(init_sentences if init_sentences else '')
    ))
    all_class.append("Instruction{0}".format(name.title()))
import pprint
print('__all__ = ', end='')
pprint.pprint(all_class)
